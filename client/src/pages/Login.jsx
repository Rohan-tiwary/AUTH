      ...
      <div className='bg-gradient-to-b from-gray-900 to-black p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form onSubmit={onSubmitHandler}>
          ...
          {state==='Sign Up' && (
             ...
             value={name}
             className='bg-transparent outline-none' type='text' placeholder='Full Name' required></input>
           </div>
          )}
          ...
          <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C] '>
            ...
            value={email}
            className='bg-transparent outline-none w-full overflow-hidden text-ellipsis whitespace-nowrap text-indigo' type='email' placeholder='Email Id' required></input>
          </div>
          ...
          <p onClick={()=>navigate('/reset-password')} className='mb-4 text-indigo-600 cursor-pointer'>Forgot Password?</p>
          <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-900 text-white font-medium'>{state}</button>
        </form>
        
        {state==='Sign Up' ? (<p className='text-gray-400 text-center text-xs mt-4'>
          Already have an account?
          <span onClick={()=>setState('login')} className='text-blue-400 cursor-pointer underline mt-2'>Login here</span>
        </p>) : 

        (<p className='text-gray-400 text-center text-xs mt-4'>
          Don't have an account?
          <span onClick={()=>setState('Sign Up')} className='text-blue-400 cursor-pointer underline mt-2'>Sign up</span>
        </p>)}
      </div>
    </div>
  )
